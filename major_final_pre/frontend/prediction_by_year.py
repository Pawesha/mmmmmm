...
def display_aggregated_graph_bar():
    ...
    if os.path.exists(csv_filename):
        try:
            ...
            fig_demand_bar = px.bar(aggregated_data, x='date', y='demand', title='Aggregated Demand vs. Date', color='date', color_discrete_sequence=colors)
            fig_demand_bar.update_xaxes(type='category', tickmode='linear', dtick=1)
            fig_demand_bar.update_yaxes(title_text='Demand')
