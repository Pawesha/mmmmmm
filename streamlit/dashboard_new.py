...
# Sidebar widgets
st.sidebar.header("Electricity Consumption Dashboard")
date_range = st.sidebar.date_input("Select Date Range", [df['DateTime'].min().date(), df['DateTime'].max().date()])
hour_range = st.sidebar.slider("Select Hour Range", 0, 23, [0, 23])
temperature_range = st.sidebar.slider("Select Temperature Range", int(df['T2M'].min()), int(df['T2M'].max()), [int(df['T2M'].min()), int(df['T2M'].max())])
...
# Create traces for each season
    season_data = []
    for seasons in ['Spring', 'Monsoon', 'Autumn', 'Winter']:
